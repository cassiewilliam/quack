...
@autotune(
    ...
    key_ignore_varlen_dim=True,  # drop ragged token dim (M down/dgrad, K wgrad) from key: persistent+CLC config is token-count-invariant
)
def gemm_tuned(
...
@autotune(
    ...
    prune_configs_by={"early_config_prune": prune_invalid_gemm_configs},
    key_ignore_varlen_dim=True,  # drop ragged token (M) dim from key: persistent+CLC config is token-count-invariant
)
def gemm_act_tuned(
