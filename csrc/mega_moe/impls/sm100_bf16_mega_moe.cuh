// ============================================================================================
// sm100_bf16_mega_moe — FP16/BF16 adaptation of sm100_fp8_fp4_mega_moe (Phase 4 EP).
// Same fused MoE forward + SymBuffer/NVLink EP dispatch/combine, but BF16xBF16 (no FP8/FP4 / MX SF).
// SF-REMOVAL SURGERY — COMPLETE (all scale-factor code paths stripped, 0 SF refs remain):
//   [done] dtypes -> bfloat16_t ; UMMA_K 32->16 ; symbol renamed.
//   [done] template params: dropped SF_BLOCK_M/N + kNumPaddedSFPoolTokens.
//   [done] layout: fp8_sf/intermediate_sf -> bf16 token/intermediate; input/l1/l2 SF buffers
//          removed + offset links rechained.
//   [done] smem: dropped smem_sfa/sfb + sf_start_ptr + amax-reduction smem; L1 CD smem -> BF16.
//   [done] tmem: dropped SFA/SFB columns (kNumTmemCols = accum only); barriers rechained after A/B.
//   [done] kernel args 10 -> 6 (dropped 4 *_sf TmaDescriptors) + prefetch.
//   [done] dispatch pull: dropped per-token SF copy; token TMA bytes = kHidden*sizeof(bf16).
//   [done] load warps: dropped SFA/SFB TMA copies; full-barrier expect_tx without SF bytes.
//   [done] MMA: block-scaled UMMA + UTCCP -> plain BF16 SM100_MMA_F16BF16_2x1SM_SS (no SF tmem).
//   [done] epilogue: dropped amax-reduce + FP8 e4m3 cast + l2_sf store; BF16 silu(g)*u stored direct.
// COMPILE STATUS: COMPILES TO SASS. qa/megamoe_bf16_check.cu (explicit instantiation,
//   nvcc -std=c++20 -gencode arch=compute_100a,code=sm_100a) -> NVCC_EXIT=0, real tcgen05 SASS.
//   BF16 RE-TILE (option A) DONE: BLOCK_K=64 (so BLOCK_K*sizeof(bf16)=128 = one swizzle atom);
//   BLOCK_K==128 assert -> BLOCK_K*sizeof(a_dtype_t)==128; L1->L2 coupling generalized to
//   kL1BlocksPerL2K = BLOCK_K/L1_OUT_BLOCK_N (FP8 2x -> BF16 1x). swizzle = min(BLOCK_K*sizeof,128).
//   (compile-check uses a finite kActivationClamp; an infinity float-NTTP breaks the __global__ stub.)
// RUNTIME STATUS (EP=2, qa/test_megamoe_bf16_ep2.py via torch symmetric memory): VALIDATED CORRECT
//   vs te_real_full_moe (real te.GroupedLinear+tex.swiglu): rc=0, rel=0.0056, cos=1.0000 (all rows).
//   Two BF16-adaptation bugs found + fixed during bring-up:
//   (1) DEADLOCK: the GEMM B (weights) load used 1x expect_tx (ported verbatim from the FP8 source)
//       but the SM100_TMA_2SM_LOAD credits the leader barrier with 2x the descriptor box for BOTH
//       operands (A split across M, B broadcast full-N to both CTAs => 2*SMEM each). Fix: B
//       expect_tx = SMEM_B*2 (matches A's existing 2x). See the two GEMM load warps below.
//   (2) L1 epilogue SwiGLU store: rewritten from the FP8 U8x4_STSM_T(b8) path to a BF16 x2 transposed
//       store-matrix (SM90_U32x2_STSM_T = stmatrix.x2.m8n8.b16.trans, added to ptx/ld_st.cuh),
//       mirroring the proven L2 x4 store. L1 N = L1_OUT_BLOCK_N = 64 bf16 = one 128B swizzle atom
//       (half of L2's 128): the 4 epilogue warps split it 16-cols-each via col = warp_in_wg*2 +
//       lane/8 (NO N-half split, x2 not x4). See the L1 epilogue below.
//   WEIGHT LAYOUT CONTRACT: the SwiGLU epilogue pairs gate/up WITHIN one TMEM load, so the 2I weight
//   columns must be gate/up INTERLEAVED at granularity 8 ([g0..7,u0..7,g8..15,u8..15,...]); plain
//   contiguous [gate(I),up(I)] mispairs (gate/up I apart spans 2 n_blocks). qa/test_megamoe_bf16_ep2
//   interleaves w1 (GLU_G=8) before launch; the host/mcore integration must do the same.
//   [done] host launcher (BF16 6 TMA descriptors + cudaLaunchKernelEx cluster grid) — qa/*.cu.
//   [done] bindings + python (torch symmetric memory) — qa/megamoe_bf16_pybind.cpp + EP=2 test.
// REMAINING: forward+backward via autograd.Function (agreed plan); port get_mega_moe_config
//   heuristics so the launch config is derived not hardcoded; mcore macro-toggle wiring.
// Track in docs/te_megamoe_phase4_ep_kernel_design.html §10.
// ============================================================================================
#pragma once
...
#include <common/profiler.cuh>

#include <common/math.cuh>
#include <common/tma_copy.cuh>
#include <common/utils.cuh>
#include <comm/barrier.cuh>
#include <layout/sym_buffer.cuh>
#include <layout/mega_moe.cuh>
#include <mma/sm100.cuh>
#include <scheduler/mega_moe.cuh>
#include <ptx/tcgen05.cuh>
#include <ptx/tma.cuh>
#include <ptx/utils.cuh>
...
sm100_bf16_mega_moe_impl(void* y,
                            ...
                            void* profiler_buffer, /* [mega_moe profiler] before grid_constant block */
    ...
    // FP16-adapt: dropped 4 *_sf TMA descriptors (l1/l2 acts_sf, weights_sf) — BF16 has no scale factors.
    ...
    // Token and buffer layouts  (FP16-adapt: all tokens are BF16, no FP8/SF buffers)
    ...
    // Registered inputs  (FP16-adapt: dropped input_sf_buffer; topk_idx chains off the token buffer)
    ...
    // L1 inputs  (FP16-adapt: dropped l1_sf_buffer)
    const auto l1_token_buffer = layout::Buffer(
        bf16_token_layout, 1, kNumMaxPoolTokens,
        input_topk_weights_buffer.get_end_ptr());
    const auto l1_topk_weights_buffer = layout::Buffer(
        l1_topk_weights_layout, 1, kNumMaxPoolTokens,
        l1_token_buffer.get_end_ptr());

    // L2 inputs  (FP16-adapt: BF16 intermediate, dropped l2_sf_buffer)
    const auto l2_token_buffer = layout::Buffer(
        bf16_intermediate_token_layout, 1, kNumMaxPoolTokens,
    ...
    using a_dtype_t = cutlass::bfloat16_t;   // FP16-adapt: was float_e4m3_t (FP8)
    using b_dtype_t = cutlass::bfloat16_t;   // FP16-adapt: was float_e2m1 (FP4)
    ...
    constexpr uint32_t UMMA_K = 16;   // FP16-adapt: was 32 (MX block); BF16 tcgen05 K=16
    ...
    // FP16-adapt: K block must give exactly one 128B swizzle atom (BLOCK_K*sizeof == 128).
    // FP8(1B): BLOCK_K=128. BF16(2B): BLOCK_K=64.  (was: BLOCK_K == 128)
    DG_STATIC_ASSERT(BLOCK_K * sizeof(a_dtype_t) == 128, "Invalid block K (need one 128B swizzle atom)");

    // Swizzle configs
    // FP16-adapt: cap at the 128B swizzle max. BLOCK_K*sizeof(bf16)=256 > 128 (FP8 was 128*1=128).
    constexpr uint32_t kSwizzleAMode = cute::min<uint32_t>(BLOCK_K * sizeof(a_dtype_t), 128u);
    constexpr uint32_t kSwizzleBMode = cute::min<uint32_t>(BLOCK_K * sizeof(b_dtype_t), 128u);
    ...
    // Shared memory sizes
    // FP16-adapt: L1 CD output is BF16 (was FP8 e4m3, BLOCK_N/2 post-SwiGLU, 2 TMA stages); L2 output BF16.
    ...
    constexpr uint32_t SMEM_SEND_BUFFER_SIZE =
        math::constexpr_align(bf16_token_layout.get_num_bytes() * kNumDispatchWarps, kSharedMemoryAlignment);
    constexpr uint32_t SMEM_A_SIZE_PER_STAGE = LOAD_BLOCK_M * BLOCK_K * sizeof(a_dtype_t);
    constexpr uint32_t SMEM_B_SIZE_PER_STAGE = LOAD_BLOCK_N * BLOCK_K * sizeof(b_dtype_t);
    constexpr uint32_t SMEM_CD_L1_SIZE =
        kNumEpilogueWarpgroups * STORE_BLOCK_M * L1_OUT_BLOCK_N * sizeof(nv_bfloat16) * kNumTMAStoreStages;
    ...
    // Tensor memory size  (FP16-adapt: no SFA/SFB tmem columns)
    ...
    const auto smem_send_buffers = layout::Buffer(
        bf16_token_layout, kNumDispatchWarps, 1,
    ...
    // FP16-adapt: dropped SF (SFA/SFB) smem and the epilogue amax-reduction smem (no FP8 quant).
    // Barriers and tensor memory pointer — placed right after the A/B GEMM smem region.
    auto barrier_start_ptr = reinterpret_cast<Barrier*>(math::advance_ptr<uint8_t>(smem_gemm_base,
    ...
    if (warp_idx < kNumDispatchWarps) {
        ...
        for (uint32_t token_idx = sm_idx * kNumDispatchWarps + warp_idx; ; token_idx += kNumGlobalWarps) {
            ...
            // TMA load token from remote rank into shared memory  (FP16-adapt: BF16 token bytes)
            ...
            // FP16-adapt: dropped the per-token SF load/store (no scale factors in BF16).
        ...
        // GEMM TMA load warp for tokens  (FP16-adapt: no SFA)
        scheduler.for_each_block([&](const sched::BlockPhase& block_phase,
            ...
            if (block_phase == sched::BlockPhase::Linear1) {
                ...
                uint64_t _spins = 0;   // [debug] localize deadlock via profiler_buffer
                while (ptx::ld_acq(ptr) != expected) {
                    if (++_spins == 400000000ull && profiler_buffer != nullptr && lane_idx == 0) {
                        int* d = reinterpret_cast<int*>(profiler_buffer);
                        d[0] = 1; d[1] = (int)ptx::ld_acq(ptr); d[2] = (int)expected;
                        d[3] = (int)pool_block_idx; d[4] = (int)sm_idx; _spins = 0;
                    }
                }
            } else {
                // The L1 output N is halved by SwiGLU into L1_OUT_BLOCK_N. Each L2 K-block (BLOCK_K)
                // consumes (BLOCK_K / L1_OUT_BLOCK_N) L1 N-blocks' arrival.
                //   FP8 : BLOCK_K=128, L1_OUT_BLOCK_N=BLOCK_N/2=64 -> 2x (was BLOCK_K==BLOCK_N).
                //   BF16: BLOCK_K=64  == L1_OUT_BLOCK_N=64          -> 1x.
                DG_STATIC_ASSERT(BLOCK_K % L1_OUT_BLOCK_N == 0, "Invalid block sizes");
                constexpr uint32_t kL1BlocksPerL2K = BLOCK_K / L1_OUT_BLOCK_N;
                const auto ptr = workspace.get_l2_arrival_mask_ptr(pool_block_idx);
                const uint32_t nbits = kL1BlocksPerL2K * num_k_blocks;
                // Full mask of `nbits` bits; guard the nbits==64 shift UB.
                const uint64_t expected = nbits >= 64u ? ~uint64_t{0} : ((1ull << nbits) - 1);
                uint64_t _spins = 0;   // [debug] localize deadlock via profiler_buffer
                while (ptx::ld_acq_gpu(ptr) != expected) {
                    if (++_spins == 400000000ull && profiler_buffer != nullptr && lane_idx == 0) {
                        int* d = reinterpret_cast<int*>(profiler_buffer);
                        d[10] = 2; d[11] = (int)ptx::ld_acq_gpu(ptr); d[12] = (int)expected;
                        d[13] = (int)pool_block_idx; d[14] = (int)sm_idx; _spins = 0;
                    }
                }
            }
            ...
            for (uint32_t k_block_idx = 0; k_block_idx < num_k_blocks; advance_pipeline(k_block_idx)) {
                ...
                // TMA copy tokens, then arrive at full barrier  (FP16-adapt: no SFA copy)
                if (cute::elect_one_sync()) {
                    tma::copy<BLOCK_K, LOAD_BLOCK_M, kSwizzleAMode, a_dtype_t>(
                        tensor_map_a_ptr, full_barriers[stage_idx], smem_a[stage_idx], k_idx, m_idx, 2);
                    // 2-CTA TMA load credits 2x the per-CTA box to the leader barrier (A is split
                    // across M: each CTA loads LOAD_BLOCK_M=BLOCK_M/2 rows; total = 2*SMEM_A).
                    if (is_leader_cta) {
                        full_barriers[stage_idx]->arrive_and_expect_tx(SMEM_A_SIZE_PER_STAGE * 2);
                    } else {
        ...
        // GEMM TMA load warp for weights  (FP16-adapt: no SFB)
        scheduler.for_each_block([&](const sched::BlockPhase& block_phase,
                                     const uint32_t& local_expert_idx,
                                     const uint32_t& num_k_blocks,
                                     const uint32_t& m_block_idx, const uint32_t& n_block_idx) {
            const auto shape_n = block_phase == sched::BlockPhase::Linear2 ? L2_SHAPE_N : L1_SHAPE_N;
            const bool is_l1 = block_phase != sched::BlockPhase::Linear2;
            ...
            for (uint32_t k_block_idx = 0; k_block_idx < num_k_blocks; advance_pipeline(k_block_idx)) {
                ...
                // TMA copy weights, then arrive  (FP16-adapt: no SFB copy)
                if (cute::elect_one_sync()) {
                    if (is_l1) {
                        // L1: gran-8 gate/up interleave delivered from CONTIGUOUS weights via the 5D
                        // descriptor (kernel epilogue unchanged). group coord = n_block*(BLOCK_N/16);
                        // 16 = 2(gu)*8(w8) rows per group.
                        tma::copy_5d<BLOCK_K, LOAD_BLOCK_N, kSwizzleBMode, b_dtype_t>(
                            &tensor_map_l1_weights, full_barriers[stage_idx], smem_b[stage_idx],
                            k_idx, 0u, 0u, n_block_idx * (BLOCK_N / 16u), local_expert_idx, 2);
                    } else {
                    ...
                    // 2-CTA TMA load credits 2x the box to the leader barrier (B broadcast: each CTA
                    // loads the full LOAD_BLOCK_N; total = 2*SMEM_B). BF16 deadlocks unless 2x.
                    if (is_leader_cta) {
        ...
        if (is_leader_cta) {
            // Make instruction descriptor  (FP16-adapt: plain BF16 UMMA, no block scaling / SF)
            // NOTES: always swap A/B
            auto instr_desc = cute::UMMA::make_instr_desc<
                b_dtype_t, a_dtype_t, float,
                UMMA_M, UMMA_N,
            ...
            scheduler.for_each_block([&](const sched::BlockPhase& block_phase,
                ...
                for (uint32_t k_block_idx = 0; k_block_idx < num_k_blocks; advance_pipeline(k_block_idx)) {
                    ...
                    if (cute::elect_one_sync()) {
                        // FP16-adapt: no UTCCP SF->TMEM copy. Issue plain BF16 UMMA.
        ...
        scheduler.for_each_block([&](const sched::BlockPhase& block_phase,
            ...
            if (block_phase == sched::BlockPhase::Linear1) {
                ...
                for (uint32_t s = 0; s < WG_BLOCK_M / STORE_BLOCK_M; ++ s) {
                    ...
                    float2 swiglu_values[kNumAtomsPerStore * 2];   // FP16-adapt: no amax_values
                    #pragma unroll
                    for (uint32_t i = 0; i < kNumAtomsPerStore; ++ i) {
                        ...
                        for (uint32_t k = 0; k < 2; ++ k) {
                            ...
                            // Clamp  (FP16-adapt: explicit float->bf162, torch disables implicit bf16 conversions)
                            if constexpr (kActivationClamp != cute::numeric_limits<float>::infinity()) {
                                const __nv_bfloat162 cpos = __float2bfloat162_rn(kActivationClamp);
                                const __nv_bfloat162 cneg = __float2bfloat162_rn(-kActivationClamp);
                                bf16_gate = __hmin2(bf16_gate, cpos);
                                bf16_up = __hmax2(bf16_up, cneg);
                                bf16_up = __hmin2(bf16_up, cpos);
                            }
                    ...
                    // FP16-adapt: store the SwiGLU result as BF16 via x2 transposed STSM (no amax / FP8
                    // quant / SF). Mirrors the proven L2 x4 BF16 store but L1 N = L1_OUT_BLOCK_N = 64 =
                    // one 128B swizzle atom (half of L2's BLOCK_N=128 = 2 atoms), so NO (warp/2) N-half
                    // split, and x2 (4 SwiGLU bf16/lane) instead of x4 (8 raw bf16/lane).
                    #pragma unroll
                    for (uint32_t i = 0; i < kNumAtomsPerStore; ++ i) {
                        // L1 N = 64 bf16 = one 128B atom = 8 col-groups; each of the 4 warps owns 2
                        // col-groups (16 bf16) via warp_idx_in_wg*2, and x2's 2 matrices (lane/8) pick
                        // the col-group within the warp's pair.
                        const uint32_t row = lane_idx % 8;
                        const uint32_t col = warp_idx_in_wg * 2 + lane_idx / 8;
                        const auto smem_ptr = smem_cd[tma_stage_idx] +
                            epilogue_wg_idx * STORE_BLOCK_M * L1_OUT_BLOCK_N * static_cast<uint32_t>(sizeof(nv_bfloat16)) +
                            i * ATOM_M * kSwizzleCDMode +
                            row * (kNumBankGroupBytes * 8) +
                            (col ^ row) * kNumBankGroupBytes;
                        // Pack the 4 SwiGLU outputs (float2 x2) into 2 bf16x2 registers for the STSM.
                        const __nv_bfloat162 p0 = __float22bfloat162_rn(swiglu_values[i * 2 + 0]);
                        const __nv_bfloat162 p1 = __float22bfloat162_rn(swiglu_values[i * 2 + 1]);
                        ptx::SM90_U32x2_STSM_T<uint32_t>::copy(
